...
def generate_path_graph():
    ...
    for room in world.rooms.items():
        exits = room[1].getExits()
        path_graph[room[0]] = dict(zip(exits, ["?"] * len(exits)))

    return path_graph


def find_path():
    path_graph = generate_path_graph()
    reverse = {"n": "s", "s": "n", "e": "w", "w": "e"}
    completed = set()
    starting_room = world.startingRoom
    # print(starting_room)
    # starting_direction = random.choice(starting_room.getExits())
    # traversalPath.append(starting_direction)
    s = Stack()
    ...
    while s.size() > 0:
        ...
        if len(current_path) > 1:
            ...
            path_graph[last_room][last_direction] = current_room
            path_graph[current_room][reverse[last_direction]] = last_room
            if "?" not in path_graph[last_room].values():
                completed.add(last_room)
            if len(path_graph[current_room].values()) == 1:
                completed.add(current_room)

        # print(current_room, path_graph[current_room].values())
